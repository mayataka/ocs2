#include <ocs2_sto_ipm/riccati_recursion/RiccatiRecursion.h>
...
void RiccatiRecursion::backwardRecursion(
  ...
  for (int i=N_-1; i>=0; --i) {
    ...
    if (phase != phaseNext) {
      ...
      backwardRecursion_.compute(riccatiPreEvent_[phase+1], dynamics[i], cost[i], 
                                 riccati_[i], lqrPolicy_[i], sto, stoNext);
    }
    else {
      backwardRecursion_.compute(riccati_[i+1], dynamics[i], cost[i],
                                 riccati_[i], lqrPolicy_[i], sto, stoNext);
    }
  }
...
void RiccatiRecursion::forwardRecursion(
  ...
  for (int i=0; i<N_; ++i) {
    ...
    ForwardRiccatiRecursion::computeState(dynamics[i], stateTrajectory[i], 
