...
void BackwardRiccatiRecursion::compute(
  ...
  AtP_.noalias() = dynamics.dfdx.transpose() * riccati_next.P;
  BtP_.noalias() = dynamics.dfdu.transpose() * riccati_next.P;
  // Factorize F
  ...
  cost.dfdu.noalias() -= dynamics.dfdu.transpose() * riccati_next.s; 
  const int nu = cost.dfdu.size();
  switch (nu)
  {
  case 3:
    Ginv_3_ = cost.dfduu.inverse();
    lqr_policy.K.noalias() = - Ginv_3_ * cost.dfdux;
    lqr_policy.k.noalias() = - Ginv_3_ * cost.dfdu;
    break;
  case 2:
    Ginv_2_ = cost.dfduu.inverse();
    lqr_policy.K.noalias() = - Ginv_2_ * cost.dfdux;
    lqr_policy.k.noalias() = - Ginv_2_ * cost.dfdu;
    break;
  case 1:
    Ginv_1_ = 1.0 / cost.dfduu.coeff(0, 0);
    lqr_policy.K.noalias() = - Ginv_1_ * cost.dfdux;
    lqr_policy.k.noalias() = - Ginv_1_ * cost.dfdu;
    break;
  default:
    llt_.compute(cost.dfduu);
    lqr_policy.K.noalias() = - llt_.solve(cost.dfdux);
    lqr_policy.k.noalias() = - llt_.solve(cost.dfdu);
    break;
  }
}
...
void BackwardRiccatiRecursion::compute(
  ...
  if (sto) {
    ...
    riccati.psi_x.noalias() += dynamics.dfdx.transpose() * riccati_next.Psi;
    riccati.psi_u.noalias() += dynamics.dfdu.transpose() * riccati_next.Psi;
    if (has_next_sto_phase) {
      riccati.phi_x.noalias() = dynamics.dfdx.transpose() * riccati_next.Phi;
      riccati.phi_u.noalias() = dynamics.dfdu.transpose() * riccati_next.Phi;
    }
    else {
    ...
    // Vtt
    Pf_.noalias() = riccati_next.P * hamiltonian.dfdt;
    riccati.xi    = hamiltonian.dfdt.dot(Pf_);
    riccati.xi   += hamiltonian.dhdt; 
    riccati.xi   += 2 * riccati_next.Psi.dot(hamiltonian.dfdt);
    ...
    if (has_next_sto_phase) {
      ...
      riccati.chi += riccati_next.Phi.dot(hamiltonian.dfdt);
    ...
    // Vt
    Pf_.noalias() = riccati_next.P * dynamics.f - riccati_next.s;
    riccati.eta   = hamiltonian.dfdt.dot(Pf_);
    riccati.eta  += hamiltonian.h;
    riccati.eta  += riccati_next.Psi.dot(dynamics.f);
    riccati.eta  += riccati.psi_u.dot(lqr_policy.k);
    riccati.eta  += riccati_next.eta;
    if (has_next_sto_phase) {
      riccati.iota  = riccati_next.Phi.dot(dynamics.f);
