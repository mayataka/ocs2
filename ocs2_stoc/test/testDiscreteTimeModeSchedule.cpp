...
TEST(testDiscreteTimeModeSchedule, testDiscreteTimeModeSchedule) {
  ...
  for (int i=0; i<isStoEnabledRef.size(); ++i) {
    EXPECT_EQ(modeSchedule.isStoEnabledAtPhase(i), isStoEnabledRef[i]);
  }
  for (int i=0; i<modeSequenceRef.size(); ++i) {
    EXPECT_EQ(modeSchedule.isStoEnabledAtTimeStage(i), isStoEnabledRef[modeSchedule.phaseAtTimeStage(i)]);
  }
...
TEST(testDiscreteTimeModeSchedule, testDiscreteTimeModeSchedule2) {
  const scalar_array_t eventTimesInput = {0.3, 0.7, 0.9, 1.5};
  const size_array_t modeSequenceInput = {1, 0, 2, 1, 0};
  const ModeSchedule modeScheduleReference(eventTimesInput, modeSequenceInput);
  const scalar_t initTime = 0.1;
  const scalar_t finalTime = 1.7;
  const scalar_t dt = 0.15;
  const auto timeDiscretization = multiPhaseTimeDiscretization(initTime, finalTime, dt, modeScheduleReference.eventTimes);
  const std::vector<bool> isStoEnabledInput = {true, false, false, true};
  DiscreteTimeModeSchedule modeSchedule(timeDiscretization, modeScheduleReference, isStoEnabledInput);
